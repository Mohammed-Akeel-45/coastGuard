...
import { genRefreshToken, hashPassword, comparePassword, hashToken, compareToken, genRandomSecret } from "./";
...
export async function register(req: Request, res: Response) {
    ...
    if (!email || !password || !user_name) return res.status(400).json({ error: "missing fields" });
...
export async function login(req: Request, res: Response) {
    ...
    await pool.query(
     ...
     VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [sessionId, user.user_id, tokenId, refreshHash, req.ip, req.get("User-Agent"), expiresAt]
    );
...
export async function refreshToken(req: Request, res: Response) {
    ...
    if (parts.length !== 2) return res.status(400).json({ error: "malformed_token" });
    ...
    try {
        await pool.query(
            "UPDATE sessions SET revoked=true, replaced_by_session_id=$1 WHERE session_id=$2",
            [newSessionId, session.session_id]
        );
        await pool.query(
            `INSERT INTO sessions (session_id, user_id, token_id, refresh_token_hash, ip_addr, user_agent, expires_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7)`,
            [newSessionId, session.user_id, newTokenId, newHash, req.ip, req.get("User-Agent"), expiresAt]
        );
